...
if (isset($_GET['id'])) {
    ...
    if ($check_result->num_rows == 0 || $quotation['quotationStatus'] != 0) {
    ...
    $cancel_quotation_stmt = $conn->prepare("UPDATE quotations SET quotationStatus = 2, updatedBy = ?, updated_at = ? WHERE quotationUId = ?");
    ...
    if ($cancel_quotation_stmt->execute()) {
        ...
        $details_stmt = $conn->prepare("UPDATE quotation_details SET status = 2, updated_at = ? WHERE referenceNumber = ?");
