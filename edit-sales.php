...
if (isset($_POST['updateSaleBTN'])) {
    ...
    try {
        ...
        // Update order
        $update_order_stmt = $conn->prepare("UPDATE orders SET customerId = ?, orderDate = ?, subTotal = ?, vat = ?, total = ?, paymentType = ?, paid = ?, due = ?, orderStatus = ?, totalProducts = ?, updated_at = ? WHERE invoiceNumber = ?");
        $update_order_stmt->bind_param("isdidsddsiss", $customerId, $orderDate, $subTotal, $vat, $grandTotal, $paymentType, $pay, $due, $paymentStatus, $totalProducts, $current_time, $invoiceNumber);
        ...
        foreach ($products as $p) {
            ...
            // Insert new detail
            $insert_detail_stmt = $conn->prepare("INSERT INTO order_details (invoiceNumber, productId, unitCost, quantity, totalCost, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
